...
def easy():
    global bombe_in_game, btn_save, score_sql, nb_score
    bombe_in_game = 15
    ...
    def onclik(i,j,plateau_de_bombe, plateau_win):
        ...
        if plateau_de_bombe[i][j]==9:
            ...
            def loose():
                ...
                frame_loose_score = tk.Frame(windows_easy)
            ...
            name_player = tk.Entry(windows_easy, font=("yellowstone", 20), borderwidth=4, fg="gray", bg="white")
            name_player.pack(pady=10)
            name_player.insert(0, "Entrez votre pseudo...")

            name_player.bind("<FocusIn>", replace_entry)

            btn_valid_entry = tk.Button(windows_easy, command=confirme_entry, text="Valider", relief="raised", bg="#B1A8A8", fg="blue", borderwidth=5, font=("Retro Gaming", 25))
            btn_valid_entry.pack(pady=10)

            frame_name_score = tk.Frame(windows_easy, bg="#B1A8A8")
        ...
        if verif(plateau_win) == True:
            ...
            def win():
                ...
                global score_sql, btn_save, new_score
                final_yes = tk.Label(windows_easy, text="WINNER", bg="#B1A8A8", fg="blue", font=("Retro Gaming", 55))
                final_yes.pack()

                frame_win_score = tk.Frame(windows_easy)
                frame_win_score.pack()

                display_score = tk.Label(frame_win_score, text="Ton Score :", bg="#B1A8A8", fg="blue",font=("Retro Gaming", 30))
                display_score.pack(side="left")
                display_score_number = tk.Label(frame_win_score, textvariable=appel_score, bg="#B1A8A8", fg="blue",font=("Retro Gaming", 30))
    ...
    cadre_interrogation = tk.Frame(windows_easy, bg="#B1A8A8")
    ...
    cadre_flags = tk.Frame(windows_easy, bg="#B1A8A8")
    cadre_flags.pack()
    ...
    cadre_suppr_affiche = tk.Frame(windows_easy, bg="#B1A8A8")
